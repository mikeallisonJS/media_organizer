...
When the user asks to check or fix review feedback on a PR:

## 1. Identify PR

From context (branch, issue number) or ask. Prefer `gh pr view --json number,url` from the current branch. Fall back to `gh api` by head branch or listing PRs.

Infer `OWNER` and `REPO` from `git remote -v` (this repo may be `mikeallisonJS/archimedius` or `mikeallisonJS/media_organizer`).
...
- Example: _"This pattern is consistent with how we handle X in `organize_plan.py`. Changing it here would diverge from the established convention without clear benefit."_
...
- Run tests before committing: `python -m pytest -q` (or targeted paths under `tests/`).
...
Add a PR comment summarising everything. **Resolve the commit SHA first** — do not rely on shell variable expansion inside single-quoted heredocs:
...
**Fixed:**
- \`metadata_extract/audio.py:42\` — [brief change description]

**Challenged:**
- \`tests/test_metadata_extract.py:30\` — [brief reasoning — see inline reply]
...
> **Important:** Do **not** use `<<'EOF'` (single-quoted heredoc) — it prevents shell variable expansion and renders `$COMMIT_SHA` as a literal string. Use `gh pr comment --body "..."` with double-quoted strings or unquoted heredocs (`<<EOF`) so that `$COMMIT_SHA` is interpolated.
